...
/* MLO (Multi-Link Operation) helper functions */
function is_mlo_iface(sid) {
	return uci.get('wireless', sid, 'mlo') == '1';
}

function get_iface_devices(sid) {
	const devices = uci.get('wireless', sid, 'device');
	if (devices == null)
		return [];
	return Array.isArray(devices) ? devices : [devices];
}

function iface_has_radio(sid, radioName) {
	const devices = get_iface_devices(sid);
	return devices.includes(radioName);
}
...
function network_updown(id, map, ev) {
	const radios = get_iface_devices(id);
	const isMlo = is_mlo_iface(id);
	let disabled = (uci.get('wireless', id, 'disabled') == '1');

	/* For MLO, check if any of the radios is disabled */
	if (!disabled) {
	...
	if (disabled) {
		...
		radios.forEach(radio => {
			...
			wifi_ifaces.forEach(wifi_iface => {
				if (iface_has_radio(wifi_iface['.name'], radio) && wifi_iface.disabled != '1')
...
var CBIWifiFrequencyValue = form.Value.extend({
	...
	setInitialValues: function(section_id, elem) {
		...
		// どのradioの設定を読み取るか判定
		// elem.dataset.radio が設定されている場合（MLOの各行）はそれを使用
		// そうでない場合は section_id の device を使用（従来の非MLO）
		const targetRadio = elem.dataset?.radio || uci.get('wireless', section_id, 'device');
		
		const htval = uci.get('wireless', targetRadio, 'htmode');
		const hwval = uci.get('wireless', targetRadio, 'hwmode');
		const chval = uci.get('wireless', targetRadio, 'channel');
		const bandval = uci.get('wireless', targetRadio, 'band');
	...
	renderWidget: function(section_id, option_index, cfgvalue) {
		...
		if (this.option && this.option.startsWith('_freq_')) {
			elem.dataset.radio = this.option.replace('_freq_', '');
		}
	...
	cfgvalue: function(section_id) {
		...
		if (this.option && this.option.startsWith('_freq_')) {
			...
			targetRadio = uci.get('wireless', section_id, 'device');
		}
		return [
		    uci.get('wireless', targetRadio, 'htmode'),
		    uci.get('wireless', targetRadio, 'hwmode') || uci.get('wireless', targetRadio, 'band'),
		    uci.get('wireless', targetRadio, 'channel')
		];
	},
	...
	write: function(section_id, value) {
		...
		uci.set('wireless', targetRadio, 'htmode', value[0] || null);

		this.useBandOption ?
			uci.set('wireless', targetRadio, 'band', value[1]) : 
			uci.set('wireless', targetRadio, 'hwmode', (value[1] == '2g') ? '11g' : '11a');
...
return view.extend({
	...
	renderOverview: function(zones) {
		...
		s.cfgsections = function() {
			const rv = [];
			const mloIfaces = [];
			const addedIfaces = new Set();

			this.radios.forEach(radio => {
				rv.push(radio.getName());

				this.wifis.forEach(wifi => {
					const wifiName = wifi.getName();
					const devices = get_iface_devices(wifiName);

					/* Skip if already added */
					if (addedIfaces.has(wifiName))
						return;

					/* MLO interface: collect for later */
					if (devices.length > 1) {
						if (!mloIfaces.includes(wifiName))
							mloIfaces.push(wifiName);
						return;
					}

					/* Non-MLO: add under the matching radio */
					if (iface_has_radio(wifiName, radio.getName())) {
						rv.push(wifiName);
						addedIfaces.add(wifiName);
					}
				});
			});

			/* Append MLO interfaces at the end */
			mloIfaces.forEach(wifiName => {
				rv.push(wifiName);
				addedIfaces.add(wifiName);
			});
		...
		s.renderRowActions = function(section_id) {
			...
			if (inst.getWifiNetworks) {
				...
				const devices = get_iface_devices(section_id);
				let isDisabled = (inst.get('disabled') == '1');

				/* Check if any of the associated radios is disabled */
				if (!isDisabled) {
					for (const dev of devices) {
						if (uci.get('wireless', dev, 'disabled') == '1') {
							isDisabled = true;
							break;
						}
					}
		...
		s.addModalOptions = function(s) {
			return network.getWifiNetwork(s.section).then(function(radioNet) {
				const devices = get_iface_devices(s.section);
				const isMlo = devices.length > 1;
				const hwtype = uci.get('wireless', devices[0], 'type');
				const have_mesh = L.hasSystemFeature('hostapd', 'mesh') || L.hasSystemFeature('wpasupplicant', 'mesh');
				let o, ss;

				/* Device Configuration - single section */
				o = s.option(form.SectionValue, '_device', form.NamedSection, devices[0], 'wifi-device', _('Device Configuration'));
				o.modalonly = true;
				...
				const isDisabled = (radioNet.get('disabled') == '1' ||
					uci.get('wireless', devices[0], 'disabled') == '1');
				...
				/* Operating frequency - one row per radio for MLO */
				devices.forEach((radioName, idx) => {
					const band = uci.get('wireless', radioName, 'band') || '';
					const bandLabel = band ? ` (${band.toUpperCase()})` : '';
					const freqLabel = isMlo
						? radioName + bandLabel + ' ' + _('Operating frequency')
						: '<br />' + _('Operating frequency');
					/* Show description only on last row */
					const freqDesc = (idx === devices.length - 1)
						? _('Some channels may be restricted to Indoor Only use by your Regulatory Domain. Make sure to follow this advice if a channel is reported as such.')
						: '';

					o = ss.taboption('general', CBIWifiFrequencyValue, '_freq_' + radioName, freqLabel, freqDesc);
					o.ucisection = s.section;
				});

				if (hwtype == 'mac80211') {
					/* Legacy rates - only for first radio (2g) */
					...
					o.depends({['_freq_' + devices[0]]: '2g', '!contains': true});

					/* Tx power - one row per radio for MLO */
					devices.forEach((radioName, idx) => {
						const band = uci.get('wireless', radioName, 'band') || '';
						const bandLabel = band ? ` (${band.toUpperCase()})` : '';
						const txLabel = isMlo
							? radioName + bandLabel + ' ' + _('Maximum transmit power')
							: _('Maximum transmit power');
						/* Show description only on last row */
						const txDesc = (idx === devices.length - 1)
							? _('Specifies the maximum transmit power the wireless radio may use. Depending on regulatory requirements and wireless usage, the actual transmit power may be reduced by the driver.')
							: '';

						o = ss.taboption('general', CBIWifiTxPowerValue, 'txpower_' + radioName, txLabel, txDesc);
						o.wifiNetwork = radioNet;

						/* Override load/write to use correct radio section */
						const targetRadio = radioName;
						o.cfgvalue = function(section_id) {
							return uci.get('wireless', targetRadio, 'txpower');
						};
						o.write = function(section_id, value) {
							uci.set('wireless', targetRadio, 'txpower', value);
						};
						o.remove = function(section_id) {
							uci.unset('wireless', targetRadio, 'txpower');
						};
					});
